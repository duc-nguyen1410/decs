...
class DoubleDiffusion(FluidModel):
    def __init__(self, params, sizes, bounds, bounded=False, dealias=3/2):
        ...
        super().__init__(params, sizes, bounds, bounded, dealias)
...
class ShearedDiffusiveConvection_StressFree(DoubleDiffusion):
    def get_IVP(self):
        ns = self._get_base_namespace()
        tau_p = self.dist.Field(name='tau_p')
        tau_u1 = self.dist.VectorField(self.coords, name='tau_u1', bases=self.x_basis)
        ...
        problem.add_equation("dz(ux)(z='left') = 0")
        if self.dim==3:
            problem.add_equation("dz(uy)(z='left') = 0")
        problem.add_equation("w(z='right') = 0")
        problem.add_equation("dz(ux)(z='right') = 0")
        if self.dim==3:
            problem.add_equation("dz(uy)(z='right') = 0")
        problem.add_equation("te(z='left') = 0")
        problem.add_equation("te(z='right') = 0")
        problem.add_equation("sa(z='left') = 0")
        problem.add_equation("sa(z='right') = 0")
